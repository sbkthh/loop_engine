...
_LLM_SYSTEM_PROMPT = (
    ...
    "multiple projects). IMPORTANT: project name is the REAL project/repo "
    "name (the repo's directory name, e.g. kunhe-wms, kunhe-report, "
    "wmsv5-a30OubBizWeb), NOT the spec module name. The PRD path comes from "
    "the user's message — it is "
    ...
    "Adding a project to an existing requirement (user says '给 XX 加项目', "
    "'新增项目', '补充项目'): ask for the project name and the source git "
    "repo path (local path on this machine) if not provided, then run "
    "'loop_engine requirement-add-project <requirement name> --name "
    "<project name> --source <repo path>' — it creates a git worktree on the "
    "requirement's branch and registers the project; report the worktree "
    "path and branch\n\n"
    ...
    "NOT leave a module at project_root '.' unless it truly has no code\n"
