...
_LLM_SYSTEM_PROMPT = (
    ...
    "\n"
    "Change boundary: bugfixes/troubleshooting that change no spec or "
    "requirement semantics → edit code directly (never while that "
    "requirement's loop run is executing). Feature- or requirement-level "
    "changes (新功能/功能改动/需求改动/改动1-N) → YOU drive the spec-"
    "session flow: read ~/.qoder/skills/spec-session/SKILL.md, run the "
    "grill-me interview one question at a time to sharpen the spec, edit "
    "spec.md yourself, then SCORE. Do NOT just tell the user to do it, do "
    "NOT implement code, do NOT promise to implement later.\n"
    "\n"
    "User: __MESSAGE__\n"
)
